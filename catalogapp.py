...
APPLICATION_NAME = "Movie List Application"
...
# Show a specific genre item
@app.route('/genres/<int:genre_id>/<int:movie_id>')
def showMovie(genre_id, movie_id):
    item = session.query(Item).filter_by(
                                         id=movie_id).first()

    if 'username' not in login_session:
        return render_template('publicMovie.html', item=item)
    else:
        return render_template('privateMovie.html', genre_id=genre_id,
                               item=item)
...
# Edit a movie
@app.route(
           '/genres/<int:genre_id>/item/<int:movie_id>/edit',
           methods=[
                    'GET',
                    'POST'])
def editMovie(genre_id, movie_id):
    ...
    if 'username' not in login_session:
        print('not')
        return redirect('/login')
    movieToEdit = session.query(Item).filter_by(id=movie_id).one()
    genre = session.query(Genre).filter_by(id=genre_id).one()
    print('before')
    print(login_session['user_id'])
    print(movieToEdit.user_id)
    if login_session['user_id'] != movieToEdit.user_id:
        flash('You are not authorized to edit this movie Please'
              'create your own movie in order to edit.')
        return redirect(url_for('showCatalog',
                                genre_id=genre_id))
    if request.method == 'POST':
        if request.form['name']:
            movieToEdit.name = request.form['name']
        if request.form['description']:
            movieToEdit.description = request.form['description']
        session.add(movieToEdit)
        session.commit()
        print('movie successfully edieted')
        return redirect(url_for('showGenre', genre_id=genre_id))
    else:
        ...
        return render_template('editMovie.html',
                               ...
                               item=movieToEdit, genres=genres)


# Delete a movie
@app.route(
           '/genres/<int:genre_id>/item/<int:movie_id>/delete',
           methods=[
                    'GET',
                    'POST'])
def deleteMovie(genre_id, movie_id):
    if 'username' not in login_session:
        return redirect('/login')
    genre = session.query(Genre).filter_by(id=genre_id).one()
    movieToDelete = session.query(Item).filter_by(id=movie_id).one()
    if login_session['user_id'] != movieToDelete.user_id:
        flash('You are not authorized to delete this movie Please'
              'create your own movie in order to delete.')
    return redirect(url_for('showCatalog',
                            genre_id=genre_id))
    if request.method == 'POST':
